    ...
    <div id="menu-container" class="main_menu">
		<div class="page-header">
			<ul class="nav navbar-nav" style="width:100%">
				...
				<li  class="active" style="width:25%"><a  href="#"class="text-error"><span class="badge-info"> 3 </span > <i class="label label-warning" style="font-size:14px;background:  #f4a62a;border-radius:0;padding:2px 10px 2px 10px"> Paiement</i></a></li>
     ...
     <div>
        <a style="background-color:transparent;color:#000405 ;width:35px;margin-left:100px;font-size:20px" type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="glyphicon glyphicon-remove-circle"></i></a>
				...
				<tbody>
					<?php if (creationPanier())
					{
					   ...
					   { 
							for($i = 0; $i < count($_SESSION['panier']['libelleProduit']); $i++ )
							{
								$description .= $_SESSION['panier']['libelleProduit'][$i].',';
								$total=MontantGlobal();
								$noOfItems=compterArticles();
								?>
									<tr>
										<td>
			...
			<input type ="hidden" value ="<?php echo $page; ?> " id="lien">
				...
				<tfoot>
				<tr>
						<th colspan="6" class="text-right ">Montant Total : <?php echo number_format(MontantGlobal(),0,'',' ');?> FCFA</th>
					</tr>
                    ...
                    <div class="panel panel-default" style="border-radius:0;border-color:#2d2925">
                      <div class="panel-heading" style=" background: #2d2925;border-color: #2d2925;border-radius:0;color:#fff">
                        <h3 class="panel-title">Mode de paiement</h3>
                      </div>
                      <div class="panel-body">
					  <form action="test_paiement.php" method="POST">
                      
						<?php 
					  while($row= mysql_fetch_array($mode_paiement)){?> 
                      
                        <label class="radio-inline" style="text-transform:capitalize">
  <input type="radio" name="mode_paiement" id="<?php echo $row['id'];?>" value="<?php echo $row['id'];?>"  required><strong> <?php echo $row['nom_sys'];?></strong>
</label><?php }?>
<div class="checkbox">
        <label >
          <input type="checkbox" value="" required> Je confirme être âgé de 18 ans ou plus. J'ai pris connaissance des CGV et de mon droit de rétractation et les accepte.
        </label>
      </div>
      ...
      <input type ="hidden" value ="<?php echo $noOfItems; ?> " name="noOfItems">
