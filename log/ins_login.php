...
if(isset ($_POST["inscription"])){
	...
	$ttt = time();
	$cod_act = mt_rand(10000, 99999) ; 
	$code_act = $cod_act."".$ttt."1" ;
	$statut=0;
	$hidden=0;
	$datjr= date("Y-m-d"); 
	$etat =mysql_query("INSERT INTO `membre` 
						(`id` ,`nom` ,`prenom` ,`civilite` ,`date_naiss` ,`adresse` ,`email` ,`tel` ,`login` ,`pwd` ,`code_act` ,
							`statut` ,`hidden` ,`date_inscription` ,`date_desincription`)
						VALUES (NULL ,'$nom', '$prenom', '$civilite', '$datenaiss', '$adresse', '$mail', '$tel',
						'$login', '$password', '$code_act', '$statut', '$hidden', '$datjr', '')");
	...
	if ($etat==true){
		...
		if (creationPanier())
			{
				...
				{ 
					header("Location:../panier/commande.php");
				}
			}
		else{
					header("Location:../index.php");
		...
		header("Location:ins_login.php?msg=1");
	}
}
elseif(isset ($_POST["connexion"])){
	$userlog=addslashes($_POST["userlog"]);
	$password=md5(addslashes($_POST["password"]));
	$ir = mysql_query("SELECT * FROM `membre` WHERE (`email` = '$userlog' or `login`='$userlog')and pwd='$password'");
	...
	if($num!=0){
		...
		$_SESSION['id']=$r_ir['id'];
		$_SESSION['login']=$r_ir['login'];
		$_SESSION['nom']=$r_ir['nom'];
		$_SESSION['prenom']=$r_ir['prenom'];
		$_SESSION['adresse']=$r_ir['adresse'];
		$_SESSION['adresse_livraison']=$r_ir['adresse'];
		$_SESSION['nom_livraison']=$nom.' '.$r_ir['prenom'];
		$_SESSION['tel_livraison']=$r_ir['tel'];
		$_SESSION['nom_correct']='correct';
		$_SESSION['tel']=$r_ir['tel'];
		...
		header("Location:ins_login.php?msg=2");
	}
}
?>
<!DOCTYPE html>
...
<a class="brand" href=""></a>
    ...
    <li style="height:20px; border-right: 1px solid #eeeeee;margin:15px 10px 0 10px;"></li>
		<li  class="" style="color:#fff"><?php if(!isset ($_SESSION['email']) ){?><a  href="log/ins_login.php" class="" style="color:#fff;margin:0 0px 0 0;padding-left:0px;padding-right:0px;">Creer un profil </a><?php } else {?><a  href="#" class="">Compte</a><?php } ?></span></li>
	 <li style="height:20px; border-right: 1px solid #eeeeee;margin:15px 10px 0 10px;"></li>
		<li  class="" style="color:#fff"><?php if(!isset ($_SESSION['email']) ){?><a  href="ins_login.php" class="" style="color:#fff;margin:0 0px 0 0;padding-left:0px;padding-right:0px;">Se connecter</a><?php } else {?><a  href="../deconnexion.php" class="" style="color:#fff">Deconnexion</a><?php } ?></span></li>
...
<!-- homepage start -->
    <div class="content homepage" id="menu-1">
		<div class="page-header">
			<ul class="nav navbar-nav" style="width:100%">
				<li  class="active" style="width:25%"><a  href="#"class="text-error"><span class="badge-info"> 1 </span > <i class="label label-warning" style="font-size:14px;background:  #f4a62a;border-radius:0;padding:2px 10px 2px 10px"> Inscription</i></a></li>
				...
				<li class="" style="width:25%"><a  href="#" ><span class="badge-info" style="background:  #2d2925;">3</span><i class="label " style="font-size:14px;color:  #2d2925;border-radius:0;padding:2px 10px 2px 10px">Paiement</i></a></li>
				<li id="" class="" style="width:25%"><a  href="#"><span class="badge-info" style="background:  #2d2925;">4</span><i class="label" style="font-size:14px;color:  #2d2925;border-radius:0;padding:2px 10px 2px 10px">Confirmation</i></a></li>
			</ul>
		...
		<div class="container" style="margin-top:4em">
        	...
        	<div class ="row">
            	<div class="col-md-6">
                	...
                	<div class="panel panel-primary panel-panier" style="border-radius:0;border-color: #d14233;">
                      <div class="panel-heading" style=" background: #d14233;border-color: #d14233;border-radius:0">
                        <h3 class="panel-title" style="font-size:15px;color:#fff">Je suis nouvelle cliente / nouveau client</h3>
                      </div>
                      ...
                      <div class="panel-body">
                        Adresse pour la facture (l'adresse de livraison peut être indiquée ultérieurement). Les champs obligatoires sont marqués *
                        <p>&nbsp;</p>
                        <form data-toggle="validator" role="form" id="myForm" action="" method="post">
                          ...
                          <div class="form-group">
                            ...
                            <input    style="border-radius: 0px;" type="text" class="form-control" id="inputName" name="nom" placeholder="Votre nom" required>
                            <div class="help-block with-errors"></div>
                          </div>
                          <div class="form-group">
                            <label for="inputPrenom" class="control-label">Prenom*</label>
                            <input style="border-radius: 0px;" type="text" class="form-control" id="inputPrenom" name="prenom" placeholder="Votre prenom" required>
                            <div class="help-block with-errors"></div>
                          </div>
                          <div class="form-group">
                            <label for="inputDateNaiss" class="control-label">Date de naissance</label>
                            <input style="border-radius: 0px;" type="text" pattern="\d{4}-\d{2}-\d{2}" class="form-control" id="inputDateNaiss" name="datenaiss" placeholder="YYYY-MM-JJ" >
                           ...
                           <div class="form-group">
                            ...
                            <input style="border-radius: 0px;" type="text" class="form-control" id="inputAdresse" name="adresse" placeholder="Ville - Quartier - Autre infos pouvant etre utiles" required>
                            <div class="help-block with-errors">Ville, Quartier, autres infos pouvant etre utiles</div>
                          </div>
                          <div class="form-group">
                            <label for="inputEmail" class="control-label">Email*</label>
                            <input style="border-radius: 0px;" type="email" class="form-control" id="inputEmail" placeholder="Email" name="mail" data-error="Email incorrect" required>
                            <div class="help-block with-errors"></div>
                          </div>
                          <div class="form-group">
                          ...
                          <div class="form-group">
                            ...
                            <input style="border-radius: 0px;" type="text" pattern="^([_A-z0-9]){3,}$" maxlength="10" class="form-control" id="inputLogin" name="login" placeholder="Votre login" required>
  						 ...
  						 <div class="form-group">
                            ...
                            <label for="inputPassword" class="control-label">Mot de passe*</label>
                              ...
                              <input style="border-radius: 0px;" type="password" data-minlength="6" class="form-control" id="inputPassword" name="password" placeholder="Mot de passe" required>
                              <span class="help-block">Minimum de 6 caracteres</span>
                           
                              <input style="border-radius: 0px;" type="password" class="form-control" id="inputPasswordConfirm" data-match="#inputPassword" data-match-error="Whoops, &ccedil;a ne correspond pas" placeholder="Confirmer votre mot de passe " required>
                              <div class="help-block with-errors"></div>
						  ...
						  <div class="form-group">
							<button  type="submit" class="btn btn-primary" style="font-size:12px;width:80px;border-radius:0;" name="inscription">Enregistrer</button>
							<button  type="reset" class="btn btn-primary" style="font-size:12px;width:80px;border-radius:0;" name="annuler">Annuler</button>
						  </div>
						</form>
                ...
                <div class="col-md-6">
                	...
                	<div class="panel panel-primary" style="border-radius:0;border-color: #d14233;">
					
                      <div class="panel-heading" style=" background: #d14233;border-color: #d14233;border-radius:0">
                        <h3 class="panel-title" style="font-size:15px;color:#fff">Je suis d&eacute;j&agrave; client(e)</h3>
                      </div>
					  <?php
						if(isset($_GET['msg'])&& $_GET['msg']==2)
						{
							echo '<div class="alert-warning"><h5>Parametres de connexion  incorrect</h5></div>';
						}
					?>
                      <div class="panel-body">
                        <form data-toggle="validator" role="form" id="myForm" action="" method="post">
                        	<div class="form-group">
                            <label for="inputName" class="control-label">Email/Login</label>
                            <input style="border-radius: 0px;" type="text" class="form-control" id="inputName" name="userlog" placeholder="Votre nom" required>
                            <div class="help-block with-errors"></div>
                          </div>
                          <div class="form-group">
                            ...
                            <label for="inputPassword" class="control-label">Mot de passe</label>
                              ...
                              <input style="border-radius: 0px;" type="password" data-minlength="6" class="form-control" id="inputPassword" placeholder="Mot de passe" required name="password">
                         ...
                         <div class="form-group">
                            <button  type="submit" class="btn btn-primary" style="font-size:12px;width:80px;border-radius:0;" name="connexion">Submit</button>
                          </div>
                        </form>
                      </div>
                    </div>
