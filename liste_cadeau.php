...
/*	if(isset($_post['occasion'])&& $_post['occasion']!=""){
	...
	if($occasion!="" && $destinataire=="" && $prix==""){
		...
		$query_cadeau =mysql_query("select * from cado_occasion where id_occasion=$occasion");
		$num=mysql_num_rows($query_cadeau);
			if($num!=0){
					while($ann = mysql_fetch_array($query_cadeau))
					{
						$id_cadeau=$ann['id_cadeau'];
	  ...
	  if($occasion=="" && $destinataire!=""&& $prix==""){
			...
			$query_cadeau =mysql_query("select * from cado_destinataire where id_destinataire=$destinataire");
			$num=mysql_num_rows($query_cadeau);
			if($num!=0){
					while($ann = mysql_fetch_array($query_cadeau))
					{
		...
		if($occasion=="" && $destinataire=="" && $prix!=""){
			...
			$array_objet = explode('-',$prix);
			$prix1 = $array_objet[0];
			$prix2 = $array_objet[1];
			if($prix1==0)
				{
					$query_cadeau =mysql_query("SELECT * FROM `cadeau`WHERE `prix` <= $prix2");
					$num=mysql_num_rows($query_cadeau);
					if($num!=0){
						while($ann = mysql_fetch_array($query_cadeau))
						{
			...
			elseif($prix2==0)
				{
					$query_cadeau =mysql_query("SELECT * FROM `cadeau`WHERE `prix`>= $prix1");
				...
				{
					$query_cadeau =mysql_query("SELECT * FROM `cadeau`WHERE `prix` >=$prix1 AND prix <= $prix2");
		...
		if($occasion!="" && $destinataire!="" && $prix==""){
				...
				$query_cadeau =mysql_query("select * from cado_occasion where id_occasion=$occasion");
				$num_verif=mysql_num_rows($query_cadeau);
				if($num_verif!=0)
				{
					while($row_num_verif= mysql_fetch_array($query_cadeau)){
							$id_cadeau_verif=$row_num_verif['id_cadeau'];
							$query_cadeau1 =mysql_query("select * from cado_destinataire where id_destinataire=$destinataire and id_cadeau =$id_cadeau_verif");
							$num_verif1=mysql_num_rows($query_cadeau1);
							if($num_verif1!=0)
							{
								while($row_num_verif1= mysql_fetch_array($query_cadeau1)){
										$id_cadeau=$row_num_verif1['id_cadeau'];
										$num3= mysql_query("select id_cadeau from $table_verif where id_cadeau = $id_cadeau");
										$num_code=mysql_num_rows($num3);
										if($num_code==0)
											{
			...
			if($occasion!="" && $destinataire=="" && $prix!=""){
				...
				{ 
					...
					if($num_verif!=0){
						while($row_num_verif = mysql_fetch_array($query_cadeau))
						{
							$id_cadeau_verif=$row_num_verif['id'];
							
							$query_cadeau1 =mysql_query("select * from cado_occasion where id_cadeau = $id_cadeau_verif and id_occasion=$occasion");
			...
			if($occasion=="" && $destinataire!="" && $prix!=""){
				...
				if($prix1==0)
				{
					...
					if($num_verif!=0){
						...
						{
							$id_cadeau_verif=$row_num_verif['id'];
							$query_cadeau1 =mysql_query("select * from cado_destinataire where id_cadeau = $id_cadeau_verif and id_destinataire =$destinataire");
				...
				if($prix1==0)
				{
					...
					if($num_verif!=0){
						while($row_num_verif = mysql_fetch_array($query_cadeau))
						{
							...
							{
								while($row_num_verif1= mysql_fetch_array($query_cadeau1)){
									$id_cadeau_verif2=$row_num_verif1['id_cadeau'];
									$query_cadeau2 =mysql_query("select * from cado_occasion where id_cadeau = $id_cadeau_verif2 and id_occasion =$occasion");
									
									$num_verif2=mysql_num_rows($query_cadeau2);
										if($num_verif2!=0)
										{
											while($row_num_verif2= mysql_fetch_array($query_cadeau2))
											{
												$id_cadeau=$row_num_verif2['id_cadeau'];
												$num3= mysql_query("select id_cadeau from $table_verif where id_cadeau = $id_cadeau");
												$num_code=mysql_num_rows($num3);
												if($num_code==0)
													{
    ...
    <div class="content homepage" id="menu-1">
		<div class="container">
          	<div class="col-md-12 col-sm-12">
            	<?php 
			if($num_row==0){echo '<div class="text-center text-warning"><h3>Aucun resultat de recherche</h3></div> ';}
				while($row_cado4 =mysql_fetch_array($result)){ 
				$id_cado4= $row_cado4['id_cadeau'];
				$query_img4=mysql_query("select * from images where id_cadeau = $id_cado4 LIMIT 0 , 1");
				$query_nom4=mysql_query("select * from cadeau where id = $id_cado4 LIMIT 0 , 1");
				$row_img4 =mysql_fetch_array($query_img4);
				$row_nom4 =mysql_fetch_array($query_nom4);
			?>
				<div class="col-md-4" >
					<div class="templatemo_botgap  templatemo_topgap gallery-item templatemo_inside">
						   ...
						   <div class="over-infos">
								<span class="product_name"><?php echo $row_nom4['nom'];?></span></br>
								<span class="product_prize" ><?php echo $row_nom4['prix'];?>&nbsp;Fcfa </span>
								<span class="product_buy pull-right" >
									<a  href="panier/ajout_panier.php?id=<?php echo $row_nom4['id'];?>&action=ajout&amp;l=<?php echo $row_nom4['nom'];?>&amp;
									lien=<?php echo $page;?>&amp;q=1&amp;img=<?php echo $row_img4['lien'];?>&amp;p=<?php echo $row_nom4['prix'];?>">Acheter </a>
								</span>
